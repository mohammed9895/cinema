<x-admin-layout>
    <div class="container px-6 mx-auto grid">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <div class="text-gray-700 dark:text-gray-400">Fullname: {{ $user->name }}</div>
            <div class="text-gray-700 dark:text-gray-400">Email: {{ $user->email }}</div>
        </div>
                ...
                <div>
                    @foreach ($user->permissions as $permission)
                        <div class="flex items-center">
                            ...
                            <button>
                                <form
                                    action="{{ route('admin.users.permissions.detach', [$user->id, $permission->id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                ...
                <div>
                    @foreach ($user->roles as $role)
                        <div class="flex items-center">
                            ...
                            <button>
                                <form action="{{ route('admin.users.roles.detach', [$user->id, $role->id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button
            ...
            <form action="{{ route('admin.users.roles', $user->id) }}" method="POST">
                ...
                <label class="block text-sm mb-3">
                    ...
                    <select name="role"
                        class="block w-full rounded mt-1 text-sm form-input {{ $errors->has('role') ? 'border-red-600 dark:text-gray-300 dark:bg-gray-700 focus:border-red-400 focus:outline-none focus:shadow-outline-red' : 'dark:border-gray-600 dark:bg-gray-700 focus:border-fuchsia-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray' }} ">
